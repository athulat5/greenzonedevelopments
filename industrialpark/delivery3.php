...
<table width="1292" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	$s=mysql_query("select * from clientorder  where indid='$indid' and ordno not in(select ordno from delivery) and ordno<>$s1");
	if(mysql_num_rows($s)==0)
	  echo "<b> No More Orders waiting for delivery";
	  else
	  {
	  $f=1;
    ...
    &nbsp; </p></td>
    <td width="9">&nbsp;</td>
    <td width="266" valign="top"><p><strong>Details</strong></p>      <?php
   ...
   ?>      <table width="260" border="0">
        <tr>
          <td width="123">Order No . </td>
          <td width="127"><?php echo $s1; ?>&nbsp;</td>
        </tr>
        <tr>
          <td>Client ID</td>
   ...
   ?>&nbsp; </p></td>
    <td width="321" valign="top"><p><strong>Client Details</strong></p>
      <?php
    ...
    <p></p></td>
    <td width="267" valign="top"><p><strong>Delivery</strong></p>      <script>
function abc()
{
...
return(false);
        ...
        <table width="263" border="0">
          <tr>
            <td width="170">Delivery Mode </td>
            <td width="83"><select name="t2" id="t2" disabled>
          ...
          <tr>
            <td colspan="2"><textarea name="t3" cols="30" id="t3" disabled><?php echo $s3; ?></textarea></td>
          </tr>
          <tr>
            <td colspan="2"><div align="center">
            <?php
			$deldate=date("Y")."-".date("m")."-".date("d");
			$s="insert into delivery values($s1,'$s2','$s3','$deldate','$staffid')";
			mysql_query($s);
			echo "<b>Delivery Entry Completed";
			   $s=mysql_query("select * from ordproducts where ordno=$s1");
   while($row=mysql_fetch_array($s))
   {
   $pid=$row[1];
   $qty=$row[2];
   $ss="update pstock set stock=stock-$qty where pid='$pid'";
   mysql_query($ss);
