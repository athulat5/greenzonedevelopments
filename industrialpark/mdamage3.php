...
<table width="1293" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td width="294" valign="top"><form name="form2" onSubmit="return abc()" method="post" action="mdamage3.ph">
    <?php
	...
	?>  <table width="300" border="0">
        <tr>
          ...
          <td width="126"><?php echo $stock-$s2;
        ...
        <tr>
          <td>Damage Qty </td>
          <td><input name="t2" type="text" id="t2" value="<?php echo $s2; ?>" disabled><?php echo $unit; ?></td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="2"><?php
		  $dno=0;
		  $s=mysql_query("select * from mdamage order by dno desc");
		  if($row=mysql_fetch_array($s))
		  {
		  $dno=$row[0];
		  }
		  ...
		  $s="insert into mdamage values($dno,'$s1','$indid',$s2,'$s3','$staffid','$ddate')";
		  mysql_query($s);
		  $s="update  mstock set stock=stock-$s2 where mname='$s1'  and indid='$indid'";
		  mysql_query($s);
		  
		  echo "<b>Successfully Moved to damage";
